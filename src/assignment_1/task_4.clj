...
;; Spec definitions --------------------------------------------------------------------------------------------------------------------------------------------
;; At the top of my file I've decided to list all my spec definitions for the data I use with all of my functions throughout this task to validate arguments.
...
;; stored them as hash-map with each one storing year, month, day and temperature. I know that records do exist within Clojure but I followed a flow-chart
...
;; for the choice is that I didn't need the polymorphism features of defrecords and also having to deal with the defrecords interface which was unnecessary with
;; the dataset provided. My get-data splits each row of the raw data into a sequence of vectors containing integers. I then format the data into my hash-map
...
;; raw data to be able to use the primitive get to retrieve the data fast, as using nth traverses the whole collection whereas get only has to do a few hops.
;; I also think that a lot of this process could be done concurrently, where multiple rows could be mapped at the same time as the order of each weather-data
;; hash-map doesn't matter at this point of the program.
...
;; this made finding the greatest and smallest variation very simple as sort-by allowed me to overwrite the default comparator of comp,so I could use Math/abs
;; to calculate and move the variation to the top of the collection and retrieve it using take.
...
;; tricky as the data don't start at the beginning of a century and doesn't end in a whole century. Luckily quotient exists in Clojure so I used this to group
;; my data by century and did the average calculations on the temperatures.
;; For each solution function I also used 0-arity overloading to make my functions re-usable, as long as the input parameters match the spec definition. If the
;; function is called with no parameters then it will fetch the already formatted data that is used to answer the questions for this task.
